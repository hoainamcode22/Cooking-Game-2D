...
public static class LevelRewardIconAutoFixer
{
    ...
    {
        // [B3 KHOÁ TOOL PHÁ — 2026-09-05] Tool này từng ghi celebrationSlots = [null×4] và tắt slot
        // nhân vật → popup lên cấp mất 4 nhân vật ăn mừng. Chặn bằng dialog, nút MẶC ĐỊNH = Huỷ.
        // DisplayDialog: nút "ok" (tham số 3) là nút được focus sẵn → đặt chữ "Huỷ" vào đó.
        bool huy = EditorUtility.DisplayDialog(
            "⚠ Tool này GHI THẲNG vào scene SCN_Farm & 29 asset LevelReward",
            "LevelRewardIconAutoFixer đã từng làm MẤT 4 nhân vật ăn mừng của popup lên cấp\n" +
            "(ghi celebrationSlots = null×4, tắt slot, ép lại unlockSlotsContainer).\n\n" +
            "Chỉ chạy khi thật sự cần gán lại icon cho LevelReward_L*.\n" +
            "Sau khi chạy PHẢI chạy lại '★ Nối lại dây popup (APPLY)' để nối dây nhân vật.\n\n" +
            "Bạn có chắc muốn chạy?",
            "Huỷ (an toàn)", "Vẫn chạy");
        if (huy) { Debug.Log("[LevelRewardIconAutoFixer] Đã huỷ theo yêu cầu — chưa đụng gì."); return; }
        FixAll(true);
    }
    ...
    public static void FixAll(bool showDialog = true)
    {
        ...
        // [B3 — 2026-09-05] Thêm đường dẫn chuẩn của tool Township (Root_HienThi/Content).
        // Trước đây content == null mà vẫn chạy tiếp → mọi `content?.Find(...) ?? content` ra null,
        // tool ghi bừa vào popup (đây là gốc lỗi mất 4 nhân vật). Nay content null ⇒ DỪNG phần scene.
        Transform content = popup != null
            ? (popup.transform.Find("PopupRoot/Panel_Dim/Content")
               ?? popup.transform.Find("PopupRoot/Content")
               ?? popup.transform.Find("Root_HienThi/Content")
               ?? popup.transform.Find("Content"))
            : null;
        if (popup != null && content == null)
        {
            report.AppendLine("✖ KHÔNG tìm thấy Content của popup (đã thử PopupRoot/Panel_Dim/Content, PopupRoot/Content, " +
                              "Root_HienThi/Content, Content) → DỪNG phần scene, KHÔNG ghi gì vào popup.");
            Debug.LogError("[LevelRewardIconAutoFixer] Không tìm thấy Content của LevelUpPopupUI → bỏ qua phần scene, " +
                           "chưa ghi gì vào popup. Dựng lại popup bằng tool Township rồi chạy lại nếu cần.");
        }
        if (popup != null && content != null)
        {
