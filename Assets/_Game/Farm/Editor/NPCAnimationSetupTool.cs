...
public static class NPCAnimationSetupTool
{
    ...
    /// <summary>Sorting mặc định cho prefab khách — trùng LivestockAI ("CongTrinh").</summary>
    ...
    public static string RunSetup(bool quiet)
    {
        ...
        try
        {
            ...
            for (int n = 1; n <= CharacterCount; n++)
            {
                ...
                // 1 · Import setting cho 12 ảnh
                Sprite[,] frames = new Sprite[Directions.Length, 3];
                bool thieuAnh = false;
                ...
                // 2 · Clip + controller (artifact sinh ra → ghi đè)
                ...
                AnimatorController controller =
                    BuildController($"{charAnimFolder}/{nv}_Tourist.controller", walkClips, idleClips);

                // 3 · Prefab (cập nhật tại chỗ, giữ chỉnh tay)
                ...
                report.AppendLine($"- {nv}: 4 clip đi + 4 clip đứng + controller · prefab {(moi ? "TẠO MỚI" : "cập nhật")}.");
                okChar++;
            }
        }
        finally
