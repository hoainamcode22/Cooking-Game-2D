...
public static class TouristBoatSetupTool
{
    ...
    private const string MenuApply12Dir = MenuRoot + "12. Ap Dung Bo Tau 12 Huong (GIU NGUYEN Waypoint & Vi Tri Keo Tay)";
    ...
    // ─────────────────────────────────────────────────────────────────────────
    //  12. ÁP DỤNG BỘ TÀU 12 HƯỚNG — GIỮ NGUYÊN 100% WAYPOINT & VỊ TRÍ ĐÃ KÉO
    // ─────────────────────────────────────────────────────────────────────────

    [MenuItem(MenuApply12Dir, false, 45)]
    public static void Apply12DirBoatsOnly()
    {
        var controllers = Object.FindObjectsByType<TouristBoatController>(FindObjectsInactive.Include, FindObjectsSortMode.None);
        if (controllers == null || controllers.Length == 0)
        {
            EditorUtility.DisplayDialog("Tourist Boat", "Không tìm thấy TouristBoatController nào trong scene.\nBạn hãy mở Scene SCN_Farm rồi thử lại.", "OK");
            return;
        }

        Undo.IncrementCurrentGroup();
        Undo.SetCurrentGroupName("Apply 12-Direction Boats Only");

        int count = 0;
        foreach (var controller in controllers)
        {
            if (controller == null) continue;
            Undo.RecordObject(controller, "Update Boat Controller Settings");

            var so = new SerializedObject(controller);
            SerializedProperty pDock = so.FindProperty("dockIndex");
            int dockIdx = pDock != null ? pDock.intValue : -1;
            if (dockIdx < 0)
            {
                string pName = controller.transform.parent != null ? controller.transform.parent.name : controller.name;
                if (pName.Contains("1") || pName.Contains("01")) dockIdx = 0;
                else if (pName.Contains("2") || pName.Contains("02")) dockIdx = 1;
                else if (pName.Contains("3") || pName.Contains("03")) dockIdx = 2;
                if (dockIdx >= 0 && pDock != null) pDock.intValue = dockIdx;
            }

            SerializedProperty pWidth = so.FindProperty("boatWorldWidth");
            if (pWidth != null) pWidth.floatValue = 260f;

            SerializedProperty pRoll = so.FindProperty("waveRollAngle");
            if (pRoll != null) pRoll.floatValue = 2.4f;

            SerializedProperty pBank = so.FindProperty("turnBankingAngle");
            if (pBank != null) pBank.floatValue = 4.5f;

            so.ApplyModifiedProperties();

            // Gán 12 sprite vào mảng
            AssignDirectionalBoatSprites(controller, dockIdx >= 0 ? dockIdx : 0);

            // Kiểm tra SpriteRenderer con 'Visual'
            Transform visualTr = controller.transform.Find("Visual");
            if (visualTr == null)
            {
                var visualGo = CreateGO("Visual", controller.transform, controller.transform.position);
                visualTr = visualGo.transform;
            }

            var sr = visualTr.GetComponent<SpriteRenderer>();
            if (sr == null) sr = Undo.AddComponent<SpriteRenderer>(visualTr.gameObject);
            sr.sortingOrder = 10;

            // Gán sprite đầu tiên cho Visual
            string prefix = (dockIdx == 1) ? "boat_red_12_dir_" : "boat_blue_12_dir_";
            Sprite s0 = AssetDatabase.LoadAssetAtPath<Sprite>($"Assets/Assetsgame/TouristBoat/12_Directions/{prefix}0.png");
            if (s0 != null)
            {
                sr.sprite = s0;
                sr.drawMode = SpriteDrawMode.Simple;
                float nativeWidth = s0.rect.width / s0.pixelsPerUnit;
                if (nativeWidth > 0.001f)
                    visualTr.localScale = Vector3.one * (260f / nativeWidth);
            }
        ...
        string msg = $"ĐÃ ÁP DỤNG BỘ TÀU 12 HƯỚNG CHO {count} TÀU DU LỊCH!\n\n" +
                     "✓ Đã gán 12 Sprite xoay 360° (Tàu Xanh cho Bến 1 & 3, Tàu Đỏ cho Bến 2).\n" +
                     "✓ Đã kích hoạt Animation Cua Quay Đầu 180° khi rời bến và dập dềnh sóng biển.\n\n" +
                     "⭐ 100% VỊ TRÍ WAYPOINT, BERTH, BẾN VÀ CÁC ĐIỂM KÉO TAY ĐƯỢC GIỮ NGUYÊN TOÀN BỘ, không bị xê dịch bất kỳ điểm nào.";
