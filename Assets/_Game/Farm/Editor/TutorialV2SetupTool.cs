...
public static class TutorialV2SetupTool
{
    ...
    // [VÒNG 14] Art NPC nay nằm ở NHIỀU đợt giao khác nhau. Quét theo THỨ TỰ ƯU TIÊN:
    // đợt MỚI NHẤT trước, cũ hơn sau — file trùng tên ở đợt sau sẽ KHÔNG ghi đè bản mới.
    // (Lỗi của Lead: prompt tổng đổi thư mục sang _MASTER nhưng quên sửa hằng số này,
    //  bấm nút nạp sẽ lấy nhầm 12 file miệng CŨ của đợt TutorialV2.)
    private static readonly string[] HANDOFF_NPC_DS =
    {
        "production/art-handoff/2026-09-04_MASTER/D_NPC_CatDeu",         // [VÒNG 16] đợt 4 ĐÃ CẮT ĐỀU 308×588
        "production/art-handoff/2026-09-04_MASTER/D_NPC_Mieng_SuaLai",   // đợt 4 gốc 512×640 (thừa 45% khung)
        "production/art-handoff/2026-09-04_TutorialV2/A_NPC_Guide",      // đợt 3 — wave/point/blink
    };

    private const string HANDOFF_VFX  = "production/art-handoff/2026-09-04_TutorialV2/B_VFX_Tutorial";

    // ── Bố cục: NPC BÊN TRÁI · CARD BÊN PHẢI (Cao ráo, cân đối giữa màn hình) ───
    private static readonly Vector2 NPC_SIZE  = new Vector2(360f, 480f);
    private static readonly Vector2 NPC_POS   = new Vector2(-460f, 50f);   // anchoredPosition, neo/pivot giữa-đáy (0.5, 0)
    private static readonly Vector2 CARD_POS  = new Vector2(-250f, 80f);   // anchoredPosition, neo/pivot (0, 0) từ giữa
    private const float CARD_WIDTH = 780f;

    // ⚠ SẾP KIỂM BẰNG MẮT: file Assets/_Game/Farm/Art/UI/TutorialV2/npc/guide_talk_01.png không đọc
    // được ở máy dev. NPC đứng BÊN TRÁI card ⇒ cô bé nhìn sang PHẢI (về phía card).
    // Nếu trong Play thấy cô bé quay LƯNG lại card thì đổi hằng này thành true rồi bấm lại nút Dựng:
    // tool lật ảnh theo trục X (localScale.x = −1) quanh tâm đáy nên chỗ đứng KHÔNG đổi.
    private static readonly bool NPC_LAT_X = false;
    ...
    public static void Dung()
    {
        ...
        npcRt.anchorMin = npcRt.anchorMax = new Vector2(0.5f, 0f);
        npcRt.pivot = new Vector2(0.5f, 0f);
        npcRt.sizeDelta = NPC_SIZE;
        npcRt.anchoredPosition = NPC_POS;
        npcRt.localScale = NPC_LAT_X ? new Vector3(-1f, 1f, 1f) : Vector3.one;
        bc.AppendLine($"║ ✔ NPC_Guide: neo giữa-đáy, đứng BÊN TRÁI card, pos {npcRt.anchoredPosition}, size {NPC_SIZE}");
        ...
        cardRt.sizeDelta = new Vector2(CARD_WIDTH, 230f);
        cardRt.anchoredPosition = CARD_POS;
        bc.AppendLine($"║ ✔ Card: neo giữa-đáy, đứng BÊN PHẢI NPC, pos {CARD_POS}, rộng {CARD_WIDTH} (tự cao theo chữ)");

        // [WP-C1] Card TỰ CAO THEO CHỮ: pivot y=0 + ContentSizeFitter dọc ⇒ card mọc LÊN TRÊN,
        // đáy giữ nguyên y=150. Tất cả GetComponent ?? AddComponent → chạy lại tool không nhân đôi.
        var cardLe = cardGo.GetComponent<LayoutElement>() ?? Undo.AddComponent<LayoutElement>(cardGo);
        cardLe.minHeight      = 230f;   // không bao giờ thấp hơn card cũ (1–2 dòng vẫn đẹp)
        cardLe.preferredWidth = CARD_WIDTH;

        var cardVlg = cardGo.GetComponent<VerticalLayoutGroup>() ?? Undo.AddComponent<VerticalLayoutGroup>(cardGo);
        cardVlg.padding = new RectOffset(46, 46, 34, 24);   // trái, phải, trên, dưới — khớp lề chữ/nút cũ
        cardVlg.spacing = 14f;
        cardVlg.childAlignment         = TextAnchor.UpperLeft;
        cardVlg.childControlWidth      = true;
        cardVlg.childControlHeight     = true;
        cardVlg.childForceExpandWidth  = true;
        cardVlg.childForceExpandHeight = false;   // KHÔNG kéo giãn con ⇒ chiều cao = đúng tổng nội dung

        var cardFit = cardGo.GetComponent<ContentSizeFitter>() ?? Undo.AddComponent<ContentSizeFitter>(cardGo);
        cardFit.horizontalFit = ContentSizeFitter.FitMode.Unconstrained;   // rộng cố định 820 theo sizeDelta
        cardFit.verticalFit   = ContentSizeFitter.FitMode.PreferredSize;

        var cardImg = cardGo.GetComponent<Image>();   // nền 9-slice vẫn nằm ngay trên Card, raycastTarget giữ nguyên
        ...
        // [WP-C1] KHÔNG neo stretch/offset nữa — VerticalLayoutGroup của Card quyết định vị trí & kích cỡ Body.
        // Đưa anchor về góc trên-trái để layout không bị lệch bởi giá trị cũ còn sót trong scene.
        bodyRt.anchorMin = bodyRt.anchorMax = new Vector2(0f, 1f);
        bodyRt.pivot = new Vector2(0f, 1f);
        bodyRt.offsetMin = bodyRt.offsetMax = Vector2.zero;

        var bodyLe = bodyGo.GetComponent<LayoutElement>() ?? Undo.AddComponent<LayoutElement>(bodyGo);
        bodyLe.minHeight      = 96f;   // ≈ 2 dòng chữ 38 — card 1 dòng vẫn giữ mặt bằng cũ
        bodyLe.flexibleHeight = 1f;

        var body = bodyGo.GetComponent<TextMeshProUGUI>();
        body.fontSize = 38f;
        body.enableAutoSizing = true;   // co nhẹ 38→28 nếu một từ quá dài, còn lại card tự cao nên vẫn giữ 38
        body.fontSizeMin = 28f;
        body.fontSizeMax = 38f;
        body.color = new Color(0.32f, 0.20f, 0.11f);   // nâu đậm, đọc rõ trên nền kem
        body.alignment = TextAlignmentOptions.TopLeft;
        body.enableWordWrapping = true;
        body.overflowMode = TextOverflowModes.Overflow;   // không cắt chữ — chiều cao do layout đo từ preferredHeight
        ...
        // ── Hàng nút (Row_Buttons) ──────────────────────────────────────────
        // [WP-C1] Hàng riêng nằm DƯỚI Body trong VerticalLayoutGroup ⇒ chữ dài bao nhiêu nút cũng bị đẩy xuống theo,
        // không còn chồng lên dòng thứ 3. HLG không điều khiển kích cỡ con ⇒ nút giữ đúng 230×68.
        var rowGo = LayHoacTao(cardGo.transform, "Row_Buttons", typeof(RectTransform));
        var rowLe = rowGo.GetComponent<LayoutElement>() ?? Undo.AddComponent<LayoutElement>(rowGo);
        rowLe.minHeight = 68f;
        rowLe.preferredHeight = 68f;
        var rowHlg = rowGo.GetComponent<HorizontalLayoutGroup>() ?? Undo.AddComponent<HorizontalLayoutGroup>(rowGo);
        rowHlg.childAlignment         = TextAnchor.MiddleRight;   // nút dồn về phải như bản cũ (-34 lề đã nằm trong padding Card)
        rowHlg.childControlWidth      = false;
        rowHlg.childControlHeight     = false;
        rowHlg.childForceExpandWidth  = false;
        rowHlg.childForceExpandHeight = false;
        rowGo.transform.SetAsLastSibling();   // luôn là con cuối của Card ⇒ nằm dưới Body

        // ── Nút Tiếp tục ────────────────────────────────────────────────────
        // Chạy lại tool trên scene cũ: Btn_Continue còn nằm thẳng dưới Card → dời vào Row_Buttons (có Undo), không tạo bản thứ 2.
        var btnCu = cardGo.transform.Find("Btn_Continue");
        if (btnCu != null && rowGo.transform.Find("Btn_Continue") == null)
        {
            Undo.SetTransformParent(btnCu, rowGo.transform, "Dời Btn_Continue vào Row_Buttons");
            bc.AppendLine("║ ✔ Dời Btn_Continue (cũ) vào Row_Buttons");
        }
        var btnGo = LayHoacTao(rowGo.transform, "Btn_Continue", typeof(RectTransform), typeof(Image), typeof(Button));
        var btnRt = (RectTransform)btnGo.transform;
        btnRt.anchorMin = btnRt.anchorMax = new Vector2(1f, 0f);
        btnRt.pivot = new Vector2(1f, 0f);
        btnRt.sizeDelta = new Vector2(260f, 68f);   // Đủ rộng cho cả nhãn "Bỏ qua bước này" / "Tiếp tục"
        btnRt.anchoredPosition = Vector2.zero;      // vị trí do HorizontalLayoutGroup sắp; giá trị cũ (-34,22) không còn ý nghĩa
        ...
        lblRt.offsetMin = new Vector2(14f, 0f); lblRt.offsetMax = new Vector2(-46f, 0f);
        ...
        chevRt.sizeDelta = new Vector2(30f, 40f);
        chevRt.anchoredPosition = new Vector2(-12f, 0f);
    ...
    private static void NapArt()
    {
        ...
        // Quét ngược danh sách: đợt CŨ copy trước, đợt MỚI copy sau và ghi đè lên
        // ⇒ bản mới nhất luôn thắng, mà các file chỉ có ở đợt cũ (wave/point/blink) vẫn giữ.
        for (int i = HANDOFF_NPC_DS.Length - 1; i >= 0; i--)
            tongCopy += CopyGoi(Path.Combine(goc, HANDOFF_NPC_DS[i]), ART_DEST_NPC,
                                $"A (NPC) — {Path.GetFileName(HANDOFF_NPC_DS[i])}", bc);
