...
{
    [SerializeField] private PigPenPopupUI pigPenPopupUI;
    ...
    private void TryOpenPigPen(Vector2 screenPos)
    {
        ...
        if (mainCamera == null)
        {
            Debug.LogError("[PigPenClickOpen] mainCamera null");
            return;
        }

        if (targetCollider == null)
        {
            Debug.LogError("[PigPenClickOpen] targetCollider null");
            return;
        }

        if (pigPenPopupUI == null)
        {
            Debug.LogError("[PigPenClickOpen] pigPenPopupUI null");
            return;
        }
        ...
        Debug.Log("[PigPenClickOpen] World = " + world2 + " | Hit = " + hit);

        if (!hit)
            return;

        Debug.Log("[PigPenClickOpen] OPEN POPUP");
        pigPenPopupUI.OpenPopup();
    }
