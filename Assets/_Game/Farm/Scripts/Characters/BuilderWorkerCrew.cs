...
public class BuilderWorkerCrew : MonoBehaviour
{
    ...
    /// <summary>
    /// [DEV-F 2026-09-03] BuilderWorker đọc THẲNG _cfg.hammerFrames/celebrateFrames (field),
    /// không có hook nào để tiêm mảng frame riêng theo từng thợ. Vì BuilderWorker.cs không
    /// nằm trong phạm vi được sửa hôm nay, ta bơm 3 bộ sprite mới bằng cách: thợ nào có bộ
    /// riêng trong workerSpriteSets thì được cấp một BuilderWorkerConfig CLONE-RUNTIME (chỉ
    /// tồn tại trong bộ nhớ, KHÔNG ghi xuống .asset) với 2 mảng frame đã tráo sang bộ riêng,
    /// mọi field khác giữ nguyên giá trị từ _cfg gốc. Danh sách này chỉ để dọn rác lúc crew
    /// bị Destroy — xem <see cref="DonDepCauHinhRieng"/>.
    /// </summary>
    private readonly List<BuilderWorkerConfig> _cauHinhRiengTheoTho = new List<BuilderWorkerConfig>();
    ...
    private BuilderWorker TaoMotThoMoi(int i)
    {
        ...
        // [DEV-F 2026-09-03] 3 bộ sprite riêng theo từng thợ (mũ vàng/cam/trắng — art thật,
        // KHÔNG còn lật/co giả để giả bộ khác nhau). "i" ở đây CHÍNH LÀ chỉ số thợ trong tổ,
        // cùng chỉ số dùng để lấy workerPrefabs[i] phía trên — nên dùng thẳng "i" để tra
        // GetHammerFrames(i)/GetCelebrateFrames(i), KHÔNG lấy chỉ số nào khác.
        BuilderWorkerConfig cfgChoTho = LayCauHinhChoTho(i);

        w.Setup(cfgChoTho, i, false, PhaseChoThoThu(i));
        return w;
    }

    /// <summary>
    /// Trả cấu hình dùng cho thợ thứ <paramref name="i"/>. Thợ có bộ sprite RIÊNG trong
    /// <c>_cfg.workerSpriteSets</c> (GetHammerFrames/GetCelebrateFrames khác bộ legacy) ⇒
    /// trả một CLONE runtime của _cfg với 2 mảng frame đã tráo sang bộ riêng đó — BuilderWorker
    /// đọc thẳng field <c>hammerFrames</c>/<c>celebrateFrames</c> nên phải tráo tại nguồn này.
    /// Thợ KHÔNG có bộ riêng (đa số cấu hình cũ, chưa có art mới) ⇒ trả THẲNG _cfg gốc, giữ
    /// nguyên hành vi hiện có (kể cả lật/co ở cấp prefab nếu Sếp còn giữ) — KHÔNG bịa cấu hình.
    /// </summary>
    private BuilderWorkerConfig LayCauHinhChoTho(int i)
    {
        Sprite[] hammerRieng    = _cfg.GetHammerFrames(i);
        Sprite[] celebrateRieng = _cfg.GetCelebrateFrames(i);

        bool coBoRieng = !ReferenceEquals(hammerRieng, _cfg.hammerFrames) ||
                         !ReferenceEquals(celebrateRieng, _cfg.celebrateFrames);
        if (!coBoRieng) return _cfg;   // legacy dùng chung — không clone, không tốn gì thêm

        BuilderWorkerConfig clone = Instantiate(_cfg);
        clone.name             = _cfg.name + "_runtime_tho" + i;
        clone.hammerFrames     = hammerRieng;
        clone.celebrateFrames  = celebrateRieng;
        _cauHinhRiengTheoTho.Add(clone);
        return clone;
    }

    /// <summary>
    /// Huỷ mọi BuilderWorkerConfig clone-runtime đã cấp cho thợ riêng (xem
    /// <see cref="_cauHinhRiengTheoTho"/>). Chỉ là ScriptableObject trong bộ nhớ — không
    /// đụng .asset gốc trong Resources. Gọi ở OnDestroy, SAU KHI toàn bộ thợ (con của
    /// GameObject này) đã tự Destroy nên không sợ NullReference giữa chừng.
    /// </summary>
    private void DonDepCauHinhRieng()
    {
        for (int i = 0; i < _cauHinhRiengTheoTho.Count; i++)
        {
            BuilderWorkerConfig c = _cauHinhRiengTheoTho[i];
            if (c != null) Destroy(c);
        }
        _cauHinhRiengTheoTho.Clear();
    }
    ...
    {
        return TinhViTri(b, padding, 0.62f);
    }

    /// <summary>
    /// [SỬA 2026-09-03 — Sếp: "cho thợ sát vào công trình một chút, còn hơi xa quá"]
    /// Bản cũ đặt thợ ở <c>min.x - padding</c> / <c>max.x + padding</c> ⇒ luôn NGOÀI rìa
    /// bounds. Với hộp quà rộng ~480 unit thì 2 thợ cách nhau ~560 unit, nhìn như đứng
    /// canh chứ không phải đang xây. Nay đo theo BÁN KÍNH: x = tâm ± (extents.x * inset
    /// + padding). inset 0.62 ⇒ thợ vào sát mép công trình, gần hơn ~45% mà vẫn không bị
    /// thân công trình vẽ đè (y-sort dùng chân thợ, xem BuilderWorker.UpdateYSort).
    /// </summary>
    public static Vector3[] TinhViTri(Bounds b, float padding, float insetRatio)
    {
        Vector3 c = b.center;
        float z = c.z;
        float ext = Mathf.Max(1f, b.extents.x);
