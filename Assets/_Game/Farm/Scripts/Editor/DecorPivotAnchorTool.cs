...
/// prefab).
///
/// 🟢 CAP NHAT VONG 13 (Tech Lead): 75 file stage_1..5.png.meta trong
/// Assets/Art/Decor/Stages/ DA DUOC SUA sang pivot DAY (spritePivot 0.5,0.5 -> 0.5,0),
/// nen 14 muc do growth quan ly KHONG con "nhay" vi tri sau khi xay xong nua.
/// Cong cu nay gio xu ly CA 75 file do o ca hai chieu (Apply va Hoan Tac) de hai ben
/// luon dong bo — hoan tac nua voi thi lech giua anh bake va anh Stages.
/// Can cu ky thuat: DecorGrowthController.cs dong 650 da tu ghi
/// "Pivot sprite la bottom-center nen offset = (0, size.y * 0.5f)" — code tieu thu
/// DA gia dinh pivot DAY tu truoc, chi co art la chua theo.
...
public static class DecorPivotAnchorTool
{
    ...
    public static void ApplyAll()
    {
        ...
        foreach (var f in CHAUHOA_FILES)
        {
            try
            {
                ...
                // 🔴 VONG 13 — TECH LEAD CHAN BUOC NAY LAI, CO Y.
                // Dich rieng GroundSprite len 138 se lam CHAU HOA di len ma BONG HOA
                // (CropPoint_1 / CropGroup / ReadyIcon la con RIENG cua root, khong phai
                // con cua GroundSprite) NAM LAI PHIA DUOI => chau roi khoi hoa.
                // Phai dich CA CUM, va sprite chau con dung chung 23+2 cho khac trong
                // Assets/_Game nen doi pivot .meta cung khong an toan.
                // De nguyen, cho mot vong rieng. Chi log ra cho Sep biet.
                _ = prefabPath;
                log.Add($"{f}: BO QUA CO Y — phai dich ca cum (chau + hoa + ReadyIcon), " +
                        "dich rieng GroundSprite la chau roi khoi hoa. Cho vong rieng.");
            }
            catch (System.Exception e)
            {
        ...
        int stages = SetStagesPivot(true);
        log.Add($"Assets/Art/Decor/Stages: da dat pivot DAY cho {stages} file stage_*.png.meta.");
        ...
        EditorUtility.DisplayDialog("Neo art decor xuong chan o",
            ...
            "Bao gom ca 75 file stage_*.png.meta trong Assets/Art/Decor/Stages/ nen 14 muc do " +
            "DecorGrowthController quan ly KHONG con nhay vi tri sau khi xay xong.\n" +
            "Chau Hoa 1-4 duoc BO QUA CO Y (phai dich ca cum chau + hoa). " +
            "Dung menu Hoan Tac neu can quay lai.",
            "OK");
    }
    ...
    public static void UndoAll()
    {
        ...
        int stagesU = SetStagesPivot(false);
        log.Add($"Assets/Art/Decor/Stages: da tra pivot GIUA cho {stagesU} file stage_*.png.meta.");
    ...
    // ══════════════════════════════════════════════════════════════════════════
    //  PIVOT CUA SPRITE ĐƠN — PHAI DI QUA TextureImporterSettings
    // ══════════════════════════════════════════════════════════════════════════
    // 🔴 LOI BIEN DICH CS1061 (vong 13, Sep bao): `TextureImporter.spriteAlignment`
    // KHONG TON TAI. Day khong phai thieu `using` — Unity chua bao gio cho property
    // do tren TextureImporter. `spriteAlignment` chi nam trong `TextureImporterSettings`,
    // doc/ghi qua `ReadTextureSettings()` / `SetTextureSettings()`.
    // (Rieng `TextureImporter.spritePivot` thi CO that, nhung no chi co hieu luc khi
    //  alignment = Custom, ma alignment lai chi dat duoc qua settings — nen phai ghi
    //  CA HAI trong cung mot lan SetTextureSettings, neu khong pivot bi bo qua.)
    //
    // Voi sprite sheet (spriteMode = Multiple) thi lai KHAC: alignment/pivot nam trong
    // tung phan tu `importer.spritesheet[i]`, khong nam trong settings. Do la ly do hai
    // nhanh "single" / "multiple" trong file nay xu ly bang hai API khac nhau.

    /// <summary>Doc alignment hien tai cua sprite ĐƠN (khong doc duoc thi tra -1).</summary>
    private static int DocAlignment(TextureImporter imp)
    {
        if (imp == null) return -1;
        var st = new TextureImporterSettings();
        imp.ReadTextureSettings(st);
        return st.spriteAlignment;
    }

    /// <summary>Dat alignment + pivot cho sprite ĐƠN roi reimport.</summary>
    private static void ApDatPivotDon(TextureImporter imp, SpriteAlignment align, Vector2 pivot)
    {
        if (imp == null) return;
        var st = new TextureImporterSettings();
        imp.ReadTextureSettings(st);
        st.spriteAlignment = (int)align;
    ...
    private static bool SetSpritePivotBottomCenter(string prefabPath, string mode, out string oldVal, out string newVal, out string texPath)
    {
        ...
        if (mode == "single")
        {
            oldVal = $"alignment={DocAlignment(importer)} pivot={importer.spritePivot}";
            ApDatPivotDon(importer, SpriteAlignment.Custom, new Vector2(0.5f, 0f));
    ...
    private static bool SetSpritePivotCenter(string prefabPath, string mode, out string oldVal, out string newVal, out string texPath)
    {
        ...
        {
            oldVal = $"alignment={DocAlignment(importer)} pivot={importer.spritePivot}";
            ApDatPivotDon(importer, SpriteAlignment.Center, new Vector2(0.5f, 0.5f));
    ...
    /// <summary>
    /// Dat pivot cho TOAN BO 75 file stage_1..5.png.meta trong Assets/Art/Decor/Stages/.
    /// Day moi la anh MA GAME THAT SU DUNG cho 14 decor co trong DecorGrowthConfig:
    /// DecorGrowthController.cs dong ~607/631 ghi thang `_sr.sprite = sp;` bang anh o day,
    /// de len anh bake trong prefab. Sua anh bake ma khong sua bo nay = vat nhay vi tri
    /// ngay khi cong trinh xay xong.
    /// </summary>
    private static int SetStagesPivot(bool bottom)
    {
        const string dir = "Assets/Art/Decor/Stages";
        if (!Directory.Exists(dir)) return 0;

        Vector2 piv = bottom ? new Vector2(0.5f, 0f) : new Vector2(0.5f, 0.5f);
        int n = 0;
        foreach (string f in Directory.GetFiles(dir, "stage_*.png", SearchOption.AllDirectories))
        {
            string path = f.Replace('\\', '/');
            var imp = AssetImporter.GetAtPath(path) as TextureImporter;
            if (imp == null) continue;
            if (DocAlignment(imp) == (int)SpriteAlignment.Custom && imp.spritePivot == piv) { n++; continue; }
            Undo.RecordObject(imp, UNDO_GROUP);
            ApDatPivotDon(imp, SpriteAlignment.Custom, piv);
            n++;
        }
        return n;
    }
