...
public class MarketBoardUI : MonoBehaviour
{
    ...
    // Mau chu gia lam moi do nguoi dung chinh trong Edit mode (lay 1 lan, giu dong bo voi Edit).
    private Color _mauChiPhiGoc;
    private bool  _daLayMauChiPhi;
    ...
    private void UpdateTimerUI()
    {
        ...
        if (textRefreshCost != null)
        {
            ...
            if (!_daLayMauChiPhi) { _mauChiPhiGoc = textRefreshCost.color; _daLayMauChiPhi = true; }
            textRefreshCost.color = affordable ? _mauChiPhiGoc : MarketBoardPalette.ButtonDisabled;
        }

        if (imageRefreshBackground != null)
        {
            bool usable = free || (manager != null && manager.CanSpendGold(cost));
            // Giu nguyen mau sprite nguoi dung chinh trong Edit mode; chi lam mo khi khong du vang.
