...
public class TutorialDialogueCard : MonoBehaviour
{
    ...
    [Tooltip("NPC đứng bên phải card (cả cụm neo góc phải-dưới, không đè hàng nút HUD).")]
    ...
    /// <summary>
    /// Các bước tutorial mà người chơi PHẢI bấm vào hàng nút HUD (BottomLeft_Nav_Group) ⇒ card
    /// KHÔNG được làm mờ hàng nút. So theo TIỀN TỐ tên step asset (TutorialManager.CurrentStepName):
    ///   • L2_01_GotoShop   — tay chỉ btn_store/btn_home, chờ WaitForOpenShop (TutorialManager ~:833)
    ///   • L2_02_UnlockCorn — thoại "Mở Shop mua ít hạt nhé!" — nếu Shop đã đóng, người chơi cần nút Shop
    /// Grep SetupTutorialL1L2Tool / SetupTutorialL2Tool: không step nào khác có targetID trỏ vào nút HUD
    /// (chỉ tutorial_plot_*, tutorial_flower_*, seed_*, shop_* trong panel Shop).
    /// </summary>
    private static readonly string[] BuocGiuHudNav = { "L2_01", "L2_02" };

    /// <summary>Alpha hàng nút HUD khi card đang mở (mờ, KHÔNG nhận tap) — không ẩn hẳn để người chơi vẫn thấy bố cục.</summary>
    private const float AlphaHudKhiMoCard = 0.35f;
    ...
    private void Awake()
    {
        ...
        var cardBtn = cardRect != null ? cardRect.GetComponent<Button>() : GetComponentInChildren<Button>();
        if (cardBtn != null && cardBtn != continueButton)
        {
            cardBtn.onClick.RemoveListener(OnCardTapped);
            cardBtn.onClick.AddListener(OnCardTapped);
        }
    ...
    {
        // Root bị tắt thẳng (không qua Hide) — VD TutorialManager SetActive(false) — vẫn phải trả HUD.
        HudNavHider.Hien(this);
        _dangGiuHudNav = false;
    }
                     ...
                     Action khiBamTiepTuc = null, Action khiChamCard = null)
    {
        ...
        // chính nút HUD (L2_01 GotoShop…) thì KHÔNG mờ — xem BuocGiuHudNav.
        CapNhatAnHudNav();
    ...
    public void Hide()
    {
        HudNavHider.Hien(this);   // trả HUD trước cả early-return: card đóng là hàng nút phải về
        _dangGiuHudNav = false;
    ...
    private void GoXong()
    {
        ...
        // [VÒNG 15] Không có callback "Tiếp tục" ⇒ bước này chờ người chơi THAO TÁC.
        // Log ra để khi tutorial đứng im còn biết vì sao không thấy nút, thay vì đoán.
        Debug.Log("[Card] Gõ xong nhưng bước này KHÔNG có nút Tiếp tục (đang chờ thao tác). " +
                  "Chạm vào card vẫn báo lên TutorialManager qua _onTap" +
                  (_onTap == null ? " — ⚠ NHƯNG _onTap CŨNG NULL, bước này không còn lối thoát nào!" : "."));
    }

    /// <summary>
    /// [VÒNG 15] LƯỚI AN TOÀN: ép hiện nút Tiếp tục dù vì lý do gì nó đang bị ẩn.
    /// TutorialManager gọi khi phát hiện bước là WaitForClick mà nút vẫn không hiện —
    /// thà hiện thừa một cái nút còn hơn để người chơi ngồi nhìn màn hình đứng im.
    /// </summary>
    public void EpHienNutTiepTuc(Action khiBam)
    {
        if (khiBam != null) _onContinue = khiBam;
        _dangGoChu = false;
    ...
    /// <summary>Nút Tiếp tục có đang hiện không (TutorialManager hỏi để biết cần cứu hay không).</summary>
    public bool NutTiepTucDangHien =>
    ...
    public void OnCardTapped()
    {
        ...
        // Bước chờ thao tác: chạm vào card sẽ ẩn card đi ngay để người chơi thao tác
        _onTap?.Invoke();
        Hide();
    }
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // Hàng nút HUD (BottomLeft_Nav_Group)
    // ═══════════════════════════════════════════════════════════════════════

    /// <summary>
    /// Mờ hàng nút HUD khi card mở, TRỪ các bước người chơi phải bấm chính nút đó.
    /// Chỉ đọc API public của TutorialManager (Instance / CurrentStepName) — không sửa file đó.
    /// </summary>
    private void CapNhatAnHudNav()
    {
        if (BuocDangCanHudNav())
        {
            HudNavHider.Hien(this);
            _dangGiuHudNav = false;
            return;
        }
        HudNavHider.An(this, AlphaHudKhiMoCard);
        _dangGiuHudNav = true;
    }

    /// <summary>Card này đang giữ HUD mờ (đã gọi An mà chưa Hien).</summary>
    private bool _dangGiuHudNav;

    /// <summary>
    /// LƯỚI AN TOÀN: TutorialManager.FinishTutorial() / SkipTutorialEntirely() KHÔNG gọi Hide()
    /// cho card (chỉ hạ alpha Tutorial_Canvas về 0) ⇒ bước cuối (L2_10_HarvestPen) xong thì root
    /// card vẫn active, OnDisable không chạy ⇒ hàng nút HUD kẹt mờ 0.35 + không nhận tap VĨNH VIỄN.
    /// Chỉ đọc DangChayTutorial (public) — tutorial hết chạy mà card còn giữ HUD thì tự trả.
    /// </summary>
    private void LateUpdate()
    {
        if (!_dangGiuHudNav) return;
        var tm = TutorialManager.Instance;
        if (tm == null || !tm.DangChayTutorial)
        {
            HudNavHider.Hien(this);
            _dangGiuHudNav = false;
        }
    ...
    private static bool BuocDangCanHudNav()
    {
        var tm = TutorialManager.Instance;
        if (tm == null) return false;

        string ten = tm.CurrentStepName;
        if (string.IsNullOrEmpty(ten)) return false;

        for (int i = 0; i < BuocGiuHudNav.Length; i++)
            if (ten.StartsWith(BuocGiuHudNav[i], StringComparison.Ordinal)) return true;
