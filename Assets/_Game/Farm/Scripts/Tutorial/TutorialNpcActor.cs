...
public class TutorialNpcActor : MonoBehaviour
{
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // TẠM THỜI (WP-A2, 2026-09-05): art wave/point/blink hiện tại còn NỀN MAGENTA và
    // KHUNG HÌNH KHÁC talk (rect khác ⇒ nhân vật nhảy kích cỡ khi đổi clip / chớp mắt).
    // ⇒ tạm dùng frame Talk cho mọi clip, tắt blink khi lệch bộ, ép nhịp chậm V3.
    // Khi đội vẽ giao GÓI A (talk/wave/point/blink cùng khung, nền trong) thì tắt 3 cờ
    // này trong Inspector là xong — KHÔNG cần sửa code.
    // ═══════════════════════════════════════════════════════════════════════
    [Header("◆ TẠM THỜI — chờ art gói A (2026-09-05)")]
    [Tooltip("Bật: yêu cầu Wave/Point đều diễn bằng frame Talk (art wave/point còn nền magenta, khung khác). " +
             "Tắt khi đội vẽ giao gói A.")]
    [SerializeField] private bool _dungTalkChoMoiClip = true;

    [Tooltip("Bật: bỏ chớp mắt khi đang dùng Talk-cho-mọi-clip, hoặc khi blinkSprite có kích cỡ rect " +
             "khác frame đang hiện (tránh nhân vật nhảy khung khi nhắm mắt).")]
    [SerializeField] private bool _tatBlinkKhiKhacBo = true;

    [Tooltip("Bật: lúc Awake ép talkFps = 6, enterSlidePixels = 0 (ghi đè giá trị cũ đã serialize trong scene). " +
             "Tắt nếu muốn dùng số trong Inspector.")]
    [SerializeField] private bool _apDungThamSoChamV3 = true;
    ...
    private void Awake()
    {
        ...
        ApDungThamSoChamV3NeuCan();
    }

    /// <summary>
    /// Tham số "chậm V3" (2026-09-05): talk 6 fps + không trượt vào. Ghi đè giá trị cũ
    /// đã serialize trong scene (12 fps / 90 px) mà không phải mở scene sửa tay.
    /// </summary>
    private void ApDungThamSoChamV3NeuCan()
    {
        if (!_apDungThamSoChamV3) return;
        talkFps          = 6f;
        enterSlidePixels = 0f;
    }
    ...
    public void Play(TutorialNpcClip clip, bool batBuocKhoiDongLai = false)
    {
        ...
        // Đang tạm dùng Talk cho mọi clip ⇒ không có "đoạn đuôi Point" để lặp.
        bool laClipPoint = clip == TutorialNpcClip.Point && !_dungTalkChoMoiClip;
        _playRoutine = StartCoroutine(ChayClip(frames, LayFps(clip), laClipPoint));
        BatChopMatNeuCan();
    }
    ...
    {
        // TẠM (2026-09-05): art wave/point còn nền magenta + khung khác talk ⇒ mọi clip dùng Talk.
        // Chờ đội vẽ giao gói A rồi tắt cờ _dungTalkChoMoiClip trong Inspector.
        if (_dungTalkChoMoiClip) return talkFrames;
    ...
    {
        // Dùng frame Talk thì cũng dùng nhịp Talk — không phát frame talk ở 14 fps của wave.
        if (_dungTalkChoMoiClip) return Mathf.Max(1f, talkFps);
    ...
    private void BatChopMatNeuCan()
    {
        ...
        // TẠM (2026-09-05): blink hiện có nền magenta + khung khác talk ⇒ khi đang dùng
        // Talk-cho-mọi-clip thì bỏ luôn chớp mắt. Lệch kích cỡ rect được kiểm thêm từng lần trong ChayChopMat.
        if (_tatBlinkKhiKhacBo && _dungTalkChoMoiClip) return;
    ...
    private IEnumerator ChayChopMat()
    {
        ...
        while (true)
        {
            ...
            // Blink khác kích cỡ frame đang hiện ⇒ nhân vật sẽ "nhảy khung" một nhịp → bỏ lần chớp này.
            if (_tatBlinkKhiKhacBo && _frameHienTai != null
                && blinkSprite.rect.size != _frameHienTai.rect.size) continue;
