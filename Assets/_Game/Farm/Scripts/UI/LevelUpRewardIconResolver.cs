...
public static class LevelUpRewardIconResolver
{
    ...
    public static Sprite Resolve(string itemId, Sprite assetIcon, string displayName = "")
    {
        ...
        // ── 3.5 · Tra cứu trực tiếp từ tất cả CropData & InventoryItemData đang nạp ─
        if (key.Length > 0)
        {
            ...
            for (int i = 0; i < allCrops.Length; i++)
            {
                var c = allCrops[i];
                if (c == null) continue;
                if (string.Equals(c.itemID, key, System.StringComparison.OrdinalIgnoreCase) ||
                    string.Equals(c.cropId, key, System.StringComparison.OrdinalIgnoreCase) ||
                    string.Equals(c.itemName, key, System.StringComparison.OrdinalIgnoreCase) ||
                    string.Equals(c.name, key, System.StringComparison.OrdinalIgnoreCase))
                {
                    Sprite s = c.harvestIcon ?? c.itemIcon ?? c.readySprite;
                    if (s != null) return s;
                }
            }

            var allItems = Resources.FindObjectsOfTypeAll<InventoryItemData>();
            for (int i = 0; i < allItems.Length; i++)
            {
                var it = allItems[i];
                if (it == null) continue;
                if (string.Equals(it.itemId, key, System.StringComparison.OrdinalIgnoreCase) ||
                    string.Equals(it.displayName, key, System.StringComparison.OrdinalIgnoreCase) ||
                    string.Equals(it.name, key, System.StringComparison.OrdinalIgnoreCase))
                {
