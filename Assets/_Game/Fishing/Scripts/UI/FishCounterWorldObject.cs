...
{
    /// <summary>
    /// QUẦY CÁ ngoài map farm — bấm vào mở FishCounterPopupUI. Chép khuôn StallWorldObject: New Input System +
    /// Collider2D.OverlapPoint (KHÔNG OnMouseDown: mobile nhiều camera không thấy chạm, và bỏ qua mọi chốt chặn).
    /// Tool FishingFarmHookSetupTool tạo object + Collider2D + component này; Sếp kéo tới chỗ muốn và thay sprite.
    /// </summary>
    public class FishCounterWorldObject : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            if (targetCollider == null) { Debug.LogWarning(FishingIds.LogTag + " FishCounterWorldObject '" + name + "' thiếu Collider2D — không bấm được."); }
        }
        ...
        private void TryOpen(Vector2 screenPos)
        {
            ...
            {
                Debug.Log(FishingIds.LogTag + " Quầy Cá mở ở cấp " + need + ".");
                if (FarmUIManager.Instance != null) { FarmUIManager.Instance.ShowHint(Loc.TF("Quầy Cá mở ở cấp {0}", need)); }
                return;
            }
            FishCounterPopupUI.Open();
        }
