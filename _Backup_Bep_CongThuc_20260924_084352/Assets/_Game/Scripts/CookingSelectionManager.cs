...
public class CookingSelectionManager : MonoBehaviour
{
    ...
    {
        // [VFX 2026-09-24] Bep V3: icon roi lach tach vao noi + noi rung rinh (KitchenJuiceFX)
        if (KitchenJuiceFX.Instance != null && KitchenJuiceFX.Instance.isActiveAndEnabled)
        {
            KitchenJuiceFX.Instance.ThaVaoNoi(card.GetMainSprite(), card.transform.position);
            return;
        }
    ...
    private void UpdateCounts()
    {
        ...
        if (ingredientsCountText != null)
            ingredientsCountText.text = Loc.TF("Chọn {0}/{1}", ingredientCount, maxIngredients);

        if (seasoningsCountText != null)
            seasoningsCountText.text = Loc.TF("Chọn {0}/{1}", seasoningCount, maxSeasonings);
    }
