...
public class WarehousePopupUI : MonoBehaviour
{
    ...
    public const float TAB_RONG = 190f, TAB_CACH = 10f;
    private static float ViTriTab(int i) => (i - 1.5f) * (TAB_RONG + TAB_CACH);   // -300,-100,100,300
    ...
    public static RectTransform DamBaoTabTatCa(RectTransform nongSan, RectTransform chanNuoi, RectTransform monAn)
    {
        ...
        for (int i = 0; i < ds.Length; i++)
        {
            ...
            if (chu != null)
            {
                ...
                if (i == 0) { crt.anchoredPosition = new Vector2(0f, crt.anchoredPosition.y); crt.sizeDelta = new Vector2(TAB_RONG - 24f, crt.sizeDelta.y); }
                else { crt.anchoredPosition = new Vector2(22f, crt.anchoredPosition.y); crt.sizeDelta = new Vector2(TAB_RONG - 70f, crt.sizeDelta.y); }
            }
            foreach (var im in r.GetComponentsInChildren<Image>(true))
                if (i > 0 && im.transform != r && im.name.Contains("Icon"))
                {
                    var irt = (RectTransform)im.transform;
                    irt.anchoredPosition = new Vector2(-(TAB_RONG * 0.5f) + 18f, irt.anchoredPosition.y);
                }
        }
        return tab;
    }
    ...
    {
        rectTabTatCa = DamBaoTabTatCa(rectTabNongSan, rectTabChanNuoi, rectTabMonAn);
