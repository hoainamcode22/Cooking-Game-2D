...
public class FarmerCrew : MonoBehaviour
{
    ...
    private void ChiaNhomRuong()
    {
        ...
        var daDung = new bool[_nongDan.Count];
        for (int n = 0; n < nhom.Count; n++)
        {
            ...
            {
                if (daDung[j] || _nongDan[j] == null) continue;
            ...
            else { f = TaoNongDan("Farmer_" + (_nongDan.Count + 1)); _nongDan.Add(f); }
            f.GiaoViec(nhom[n], false);
        }
        for (int j = 0; j < daDung.Length; j++)
