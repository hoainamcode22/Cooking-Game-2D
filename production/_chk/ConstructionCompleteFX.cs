...
public class ConstructionCompleteFX : MonoBehaviour
{
                                ...
                                System.Action onReveal)
    {
        ...
        if (reusableVfxPrefab != null)
        {
            ...
            // [FIX-HOPQUA 2026-09-02] Prefab Lana ship với sorting Default/0 → bị công
            // trình (Objects/500+) vẽ đè, pháo hoa "tàng hình" (CHAN_DOAN_PHAOHOA
            // NGUYÊN NHÂN 1). Ép lại cho TỪNG ParticleSystemRenderer — đúng pattern
            // LevelUpPopupUI.cs:794-800, layer resolve an toàn qua TouristSortingLayers.
            string vfxLayer = TouristSortingLayers.Resolve(TouristSortingLayers.Overlay);
            int vfxOrder = 1000;
            foreach (ParticleSystemRenderer psr in vfx.GetComponentsInChildren<ParticleSystemRenderer>(true))
            {
                psr.sortingLayerName = vfxLayer;
                psr.sortingOrder     = vfxOrder++;
            }
