...
public class LevelUpGiftSlotUI : MonoBehaviour
{
    ...
    /// <summary>
    /// [R2 ICON] Placeholder khi icon = null SAU KHI đã tra hết chuỗi fallback
    /// (LevelUpRewardIconResolver). Bật lại Image icon dưới dạng đĩa màu phẳng
    /// (màu suy từ id — cùng id luôn cùng màu, nhìn vẫn phân biệt được) thay vì
    /// để ô trống chỉ có chữ — chính là cái ảnh chụp bị Sếp chê.
    /// Resolver đã log warning [LevelUp] MỘT LẦN cho id đó, ở đây không log nữa.
    /// </summary>
    public void ShowPlaceholderTint(Color color)
    {
        if (iconImage == null) return;
        iconImage.sprite  = null;      // Image không sprite → vẽ khối màu đặc
