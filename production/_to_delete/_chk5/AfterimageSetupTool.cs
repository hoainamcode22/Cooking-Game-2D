...
public static class AfterimageSetupTool
{
    ...
    /// <summary>
    /// [THÊM 2026-09-03] Gắn tag cho object ĐANG CHỌN trong Hierarchy.
    /// Lý do tồn tại: object "NPC_Villagers" trong SCN_Farm KHÔNG có con nào (Lead xác minh
    /// bằng cách đọc scene YAML) — bà lão hàng rông / quân nhân / nhân viên tàu nằm ở nhánh
    /// khác hoặc là prefab instance. Nên menu quét theo tên không tới được họ; Sếp click chọn
    /// từng người (hoặc Ctrl+click nhiều người) rồi bấm menu này.
    /// Đặt luôn minSpeedOverride = 20 u/s vì NPC cảnh đi lững thững, ngưỡng chung 60 quá cao.
    /// </summary>
    [MenuItem(MenuRoot + "★ Gắn tag cho object ĐANG CHỌN (Selection)", false, 11)]
    public static void TagSelection()
    {
        ...
        {
            EditorUtility.DisplayDialog("Afterimage",
                "Chưa chọn object nào.\n\nMở SCN_Farm → click chọn nhân vật trong Hierarchy " +
                "(bà lão hàng rông / quân nhân / nhân viên tàu — Ctrl+click để chọn nhiều) " +
                "→ bấm lại menu này.", "OK");
            return;
        }

        List<string> gan = new List<string>();
        List<string> daCo = new List<string>();
        List<string> khongSprite = new List<string>();

        for (int i = 0; i < sel.Length; i++)
        {
            GameObject go = sel[i];
            if (go == null) continue;
            if (go.GetComponentInChildren<SpriteRenderer>(true) == null) { khongSprite.Add(go.name); continue; }
            if (go.GetComponent<AfterimageTag>() != null) { daCo.Add(go.name); continue; }

            AfterimageTag tag = Undo.AddComponent<AfterimageTag>(go);
            tag.includeChildren = true;
            tag.minSpeedOverride = 20f;   // NPC cảnh đi chậm
            EditorUtility.SetDirty(go);
            gan.Add(go.name);
        }

        if (gan.Count > 0 && sel[0] != null)
            EditorSceneManager.MarkSceneDirty(sel[0].scene);

        Debug.Log("[Afterimage] Selection: gắn tag cho " + gan.Count + " object" +
                  (gan.Count > 0 ? ": " + string.Join(", ", gan) : "") +
                  (daCo.Count > 0 ? " | đã có sẵn: " + string.Join(", ", daCo) : "") +
                  (khongSprite.Count > 0 ? " | BỎ QUA (không có SpriteRenderer): " + string.Join(", ", khongSprite) : "") +
                  (gan.Count > 0 ? " | Scene dirty — NHỚ Ctrl+S. minSpeedOverride=20 u/s (sửa được trong Inspector)." : ""));
    }
