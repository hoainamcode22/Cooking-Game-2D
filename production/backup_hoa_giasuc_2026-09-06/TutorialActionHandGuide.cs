...
public class TutorialActionHandGuide : MonoBehaviour
{
    ...
    private IEnumerator SweepRoutine(string[] ids, bool needReady)
    {
        ...
        while (true)
        {
            ...
            float hold = 0f;
            const float holdDur = 1.1f;   // [FIX] 0.45 -> 1.1: tay dung lai du lau, bot dao dien
            while (hold < holdDur)
            {
            ...
            if (nxtRT != null && nxt != cur)
            {
                ...
                float elapsed = 0f;
                const float dur = 0.55f;   // [FIX] luot cham hon mot chut cho mem mat
                while (elapsed < dur)
                {
                    elapsed += Time.unscaledDeltaTime;
    ...
    /// <summary>
    /// [FIX 2026-09-06] Bam tay MUOT toi target thay vi snap cung moi khung hinh.
    /// Snap cung lam tay giat khi camera con dang lia hoac khi o proxy dich chuyen —
    /// dung la cam giac "khung tay" ma Sep bao. Lan dau van dat thang de khong bi bay tu xa toi.
    /// </summary>
    private void PointAtMuot(RectTransform target, float toc = 16f)
    {
        bool show = target != null && target.gameObject.activeInHierarchy;
        if (!show) { _hand.gameObject.SetActive(false); return; }

        Vector3 dich = TargetCenter(target);

        if (!_hand.gameObject.activeSelf || !_hasLastFingertipWorld)
        {
            _hand.gameObject.SetActive(true);
            PlaceHandFingertipAt(dich);
            return;
        }

        float k = 1f - Mathf.Exp(-toc * Time.unscaledDeltaTime);
        PlaceHandFingertipAt(Vector3.Lerp(_lastFingertipWorld, dich, k));
    }
