...
public class TutorialRuntimeTargetResolver : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        StartCoroutine(QuetOMoKhoaLoop());   // [FIX 2026-09-06] o dat / chau hoa mo khoa SAU khi vao game
    ...
    {
        // [FIX 2026-09-06] Nut CUA HANG tren HUD phai dang ky TRUOC (buoc L2_01 can no khi
        // shop CHUA mo). Truoc day code chi tay vao "btn_store"/"btn_home" — 2 ten khong
        // bao gio duoc dang ky ⇒ buoc 20 khong co ban tay nao.
        DangKyNutShopTrenHud();
    ...
    /// <summary>
    /// [FIX 2026-09-06] Dang ky nut CUA HANG cua HUD lam target tutorial.
    /// Alias: "hud_shop" (ten moi) + "btn_store" (ten cu ma TutorialManager dang goi).
    /// Tim theo TownshipHUDController.btnTabShop, fallback tim object ten "Tab_Shop".
    /// </summary>
    private void DangKyNutShopTrenHud()
    {
        if (TutorialManager.GetTargetRect("hud_shop") != null) return;

        GameObject nutShop = null;

        var hud = Object.FindFirstObjectByType<FarmGame.UI.TownshipHUDController>(FindObjectsInactive.Include);
        if (hud != null && hud.btnTabShop != null) nutShop = hud.btnTabShop.gameObject;
        ...
        if (nutShop == null)
        {
            ...
            if (canvasHud != null)
            {
                foreach (var t in canvasHud.GetComponentsInChildren<Transform>(true))
                {
                    if (t.name == "Tab_Shop") { nutShop = t.gameObject; break; }
                }
            }
        }

        if (nutShop == null) return;

        AddRuntimeTarget(nutShop, "hud_shop");
        AddRuntimeTarget(nutShop, "btn_store");
        Debug.Log("[TutorialTargetResolver] Da dang ky nut CUA HANG (hud_shop / btn_store).");
    }
    ...
    /// <summary>
    /// [FIX 2026-09-06] LOI GOC cua "phan hoa khong co ban tay":
    /// SetupRicePlotProxy/SetupFlowerPotProxy chi chay DUNG MOT LAN, mot khung hinh sau Start().
    /// Luc do nguoi choi con cap 1 nen CHAU HOA CHUA MO KHOA ⇒ LayChauHoa() rong ⇒ khong proxy
    /// "tutorial_flower_xx" nao duoc tao. Len cap 2 mo khoa chau thi khong ai dang ky lai nua,
    /// nen ca giai doan trong hoa vinh vien khong co tay chi dan (va gate cung khong co o de dem).
    /// Nay: quet moi 0,5s, so o/chau DA MO KHOA thay doi thi dang ky bu ngay.
    /// CreateWorldProxy tu chong trung (_worldProxies.ContainsKey) nen goi lai hoan toan an toan.
    /// </summary>
    private IEnumerator QuetOMoKhoaLoop()
    {
        var wait = new WaitForSeconds(0.5f);
        int soOLuaCu = -1, soChauHoaCu = -1;

        while (true)
        {
            yield return wait;
            if (_tutorialCanvas == null) continue;

            int soOLua   = TutorialStepTriggerBridge.LayODatLua().Count;
            int soChauHoa = TutorialStepTriggerBridge.LayChauHoa().Count;

            if (soOLua != soOLuaCu || soChauHoa != soChauHoaCu)
            {
                var bridge = GetComponent<TutorialStepTriggerBridge>();
                if (soOLua != soOLuaCu)
                {
                    SetupRicePlotProxy(bridge);
                    Debug.Log($"[TutorialTargetResolver] O dat mo khoa: {soOLuaCu} -> {soOLua}, da dang ky lai tutorial_plot_xx.");
                }
                if (soChauHoa != soChauHoaCu)
                {
                    SetupFlowerPotProxy(bridge);
                    Debug.Log($"[TutorialTargetResolver] Chau hoa mo khoa: {soChauHoaCu} -> {soChauHoa}, da dang ky lai tutorial_flower_xx.");
                }
                soOLuaCu    = soOLua;
                soChauHoaCu = soChauHoa;
            }
        }
    }
