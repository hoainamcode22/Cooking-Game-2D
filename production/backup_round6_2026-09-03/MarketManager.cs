...
public class MarketManager : MonoBehaviour
{
    ...
    /// <summary>
    /// [FIX 2026-09-02 — lệnh Sếp] Vào game KHÔNG bật sẵn bảng tin chợ.
    ...
    /// lần đầu tiên NGƯỜI CHƠI mở → đóng sập ngay trước mặt họ): chỉ tự đóng trong
    /// 2 giây đầu sau khi load scene. Nếu mai này scene được lưu ở trạng thái tắt,
    /// Start() sẽ chạy lúc người chơi mở popup lần đầu — khi đó đã quá 2s nên
    /// dòng đóng KHÔNG chạy, popup mở bình thường.
    /// Đóng "câm": SetActive(false) thẳng, không animation, không toast.
    /// </summary>
    private void Start()
    {
        if (closeBoardOnSceneStart && IsOpen && Time.timeSinceLevelLoad < 2f)
        {
