...
public class LandExpansionManager : MonoBehaviour
{
    ...
    [Tooltip("Bat = khi VAO GAME thi AN het hang rao chia lo, chi con bien chi duong. " +
    ...
    /// <summary>An hang rao chia lo khi vao game (designer van thay trong Editor).</summary>
    public void ApplyFenceVisibility()
    {
        if (!Application.isPlaying || !hideFenceInPlayMode) return;
        var t = designerFenceTilemap;
        if (t == null)
        {
            var grid = GameObject.Find(IsoGrid.IsoGridObjectName);
            if (grid != null)
            {
                var f = grid.transform.Find("Tilemap_IsoFence");
                if (f != null) t = f.GetComponent<Tilemap>();
            }
        }
        if (t == null) return;
        var r = t.GetComponent<TilemapRenderer>();
        if (r != null) r.enabled = false;   // an mat nhung tile van con de tinh vung
        designerFenceTilemap = t;
    }
    ...
    private void RespawnSigns()
    {
        ...
        foreach (var r in regions)
        {
            ...
            // uu tien bien chi duong moi; van ho tro bien cu neu prefab dung script cu
            var board = go.GetComponent<LandRegionSignBoard>();
            if (board == null && go.GetComponent<LandRegionSign>() == null)
                board = go.AddComponent<LandRegionSignBoard>();
            if (board != null) board.Bind(r, this);
            else
            {
                var old = go.GetComponent<LandRegionSign>();
                if (old != null) old.Bind(r, this);
            }
            spawnedSigns.Add(go);
        }
    }
