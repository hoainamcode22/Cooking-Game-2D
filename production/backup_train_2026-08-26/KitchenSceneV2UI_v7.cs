...
namespace KitchenUIv2
{
    ...
    public class KitchenSceneV2UI : MonoBehaviour
    {
        ...
        private void BindExistingHierarchy()
        {
            ...
            if (_ovenRect != null)
            {
                ...
                if (sbar != null)
                {
                    ...
                    if (f2 != null) _imgOvenFill = f2.GetComponent<Image>();
                    var st = sbar.Find("Txt_State");
                    if (st != null) _txtOvenState = st.GetComponent<TMP_Text>();
                }
            }
            ...
            // ── R7 (2026-08-27): plaque lò + decor kho/lò — bản Hierarchy cũ chưa có thì tự tạo 1 lần ──
            var stateBarT = F("Oven/Oven_StateBar");
            if (stateBarT != null && skin.plaqueOvenState != null)
            {
                var sbImg = stateBarT.GetComponent<Image>();
                if (sbImg != null && sbImg.sprite != skin.plaqueOvenState)
                {
                    Skin9(stateBarT.gameObject, skin.plaqueOvenState);
                    stateBarT.sizeDelta = new Vector2(240f, 52f); // khớp tỉ lệ art 300x64
                }
            }
            if (transform.Find("Deco_Crates") == null)
                MakeDecor(_root, "Deco_Crates", skin.decoCrateStack, 1f, 0.5f, new Vector2(-175f, 65f), new Vector2(70f, 62f), new Vector2(1f, 0.5f));
            if (transform.Find("Deco_Firewood") == null)
                MakeDecor(_root, "Deco_Firewood", skin.decoFirewood, 1f, 1f, new Vector2(-300f, -315f), new Vector2(62f, 50f), new Vector2(1f, 1f));
        ...
        private void BuildStage()
        {
            ...
            // Hộp VÀO KHO + Thùng gỗ decor cạnh kho
