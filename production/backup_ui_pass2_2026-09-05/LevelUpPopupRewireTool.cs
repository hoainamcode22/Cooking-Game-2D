...
public static class LevelUpPopupRewireTool
{
    ...
    private static void Chay(bool ghiThat)
    {
        ...
        bc.AppendLine("║");
        bc.AppendLine("║ ── ④ Nền mờ · nút Tiếp tục · dây chữ (unlockDescText / hintText)");
        ...
        // ④c · Dây chữ — tạo TMP dưới Content nếu chưa có (neo đáy Content, popup runtime tự bật/tắt)
        // Vị trí an toàn: Text_MoKhoa y=215, Text_Hint y=165 (Nút Tiếp Tục ở y=78, cao 118 -> đỉnh nút ở y=137)
        NoiDayChu(soPopup, popup, contentTf, "unlockDescText", "Text_MoKhoa", new Vector2(0f, 215f), 28, bc, ref soSua, ref soLoi, ghiThat);
        NoiDayChu(soPopup, popup, contentTf, "hintText",       "Text_Hint",   new Vector2(0f, 165f), 24, bc, ref soSua, ref soLoi, ghiThat);
    ...
    private static void NoiDayChu(
        ...
        string fieldName, string tenObj, Vector2 viTri, int fontSize,
        StringBuilder bc, ref int soSua, ref int soLoi, bool ghiThat)
    {
        ...
        if (tmp != null)
        {
            ...
            if (existingRt.anchoredPosition != viTri)
            {
                if (ghiThat)
                {
                    Undo.RecordObject(existingRt, "Cập nhật toạ độ " + tenObj);
                    existingRt.anchorMin = existingRt.anchorMax = new Vector2(0.5f, 0f);
                    existingRt.pivot = new Vector2(0.5f, 0.5f);
                    existingRt.anchoredPosition = viTri;
                    existingRt.sizeDelta = new Vector2(900f, 40f);
                    EditorUtility.SetDirty(existingRt);
                }
            }
            soSua++;
            bc.AppendLine($"║    ✔ nối & căn chỉnh {fieldName} → '{tenObj}' (vị trí y={viTri.y})");
            prop.objectReferenceValue = tmp;
            return;
        }

        soSua++;
        bc.AppendLine($"║    ✔ tạo TMP '{tenObj}' dưới Content (neo đáy, y={viTri.y}) rồi nối {fieldName}" +
        ...
        go.SetActive(false);   // LevelUpPopupUI.PopulateUI tự bật khi có chữ
