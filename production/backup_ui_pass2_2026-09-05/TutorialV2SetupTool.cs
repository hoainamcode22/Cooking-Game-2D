...
public static class TutorialV2SetupTool
{
    ...
    private static void Dung()
    {
        ...
        // [VÒNG 15 — Sếp chốt] ĐỂ HÀNG NÚT CHE CHỖ CẮT.
        //
        // Ảnh NPC gốc vốn CẮT NGANG HÔNG (nội dung chạm mép dưới canvas y=640) — đó là
        // thiết kế của bộ art, không sửa được bằng toạ độ. Vòng 14 tôi nâng NPC lên y=210
        // để hết bị đè, nhưng thế là chỗ cắt LỘ RA lơ lửng giữa màn hình, nhìn còn tệ hơn.
        //
        // Cách đúng: dìm chân NPC xuống DƯỚI mép màn hình, và để hàng nút CỬA HÀNG/KHO
        // (Canvas_HUD, sortingOrder cao hơn) vẽ đè lên phần dưới ⇒ chỗ cắt khuất hoàn toàn.
        //
        // Số đo (canvas quy đổi 1920×1080):
        //   hàng nút BottomLeft_Nav_Group : x −347→409 · y −74→118
        //   card thoại (sau khi dời)      : x  730→1550 · y 150→380
        //   NPC                           : x  420→720  · y −40→335
        //     → hở hàng nút 11px bên trái, hở card 10px bên phải, KHÔNG đè cái nào
        //     → phần y −40→118 bị hàng nút che ⇒ không còn thấy chỗ cắt
        //     → phần nhìn thấy y 118→335 = 217px, đủ rõ mặt và thân trên
        npcRt.sizeDelta = new Vector2(300f, 375f);
        npcRt.anchoredPosition = new Vector2(420f, -40f);
        ...
        // [VÒNG 15] Thu 880→820 và dời 90→180 để chừa đúng chỗ cho NPC bên trái:
        // card trái = 960 + 180 − 410 = 730, mà NPC kết thúc ở 720 ⇒ hở 10px, không chồng.
        // Giữ y=150 (trên hàng nút, đỉnh 118).
        cardRt.sizeDelta = new Vector2(820f, 230f);   // chiều cao chỉ là giá trị khởi đầu — ContentSizeFitter sẽ ghi đè
        cardRt.anchoredPosition = new Vector2(180f, 150f);

        // [WP-C1] Card TỰ CAO THEO CHỮ: pivot y=0 + ContentSizeFitter dọc ⇒ card mọc LÊN TRÊN,
        // đáy giữ nguyên y=150 nên không đè hàng nút HUD. Tất cả GetComponent ?? AddComponent → chạy lại tool không nhân đôi.
        ...
        btnRt.sizeDelta = new Vector2(230f, 68f);   // HLG childControl=false ⇒ đọc đúng kích cỡ này
