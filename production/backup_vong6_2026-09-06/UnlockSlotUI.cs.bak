...
public class UnlockSlotUI : MonoBehaviour
{
    ...
    private void SetupCore(Sprite icon, string tagText, Color tagColor, string caption)
    {
        ...
        if (captionText != null)
        {
            ...
            if (has)
            {
                ...
                // [V6 2026-09-05] Ép kiểu chữ tên: 1 dòng, tự co 12–18, dài quá thì "…"
                // (áp cả cho caption do tool/prefab gán sẵn, không chỉ caption tự tạo).
    ...
    private TextMeshProUGUI CreateCaptionTextRuntime()
    {
        ...
        // [V6 2026-09-05] Neo (0.5,0) = MÉP DƯỚI ô, pivot đỉnh. Bản cũ trừ thêm h/2 nữa
        // → caption rơi ~99px DƯỚI ô, bị RectMask2D của Viewport khung trắng cắt sạch,
        // tên vật phẩm chưa bao giờ hiện. Nay đặt sát mép dưới (-2px), cao 26px, 1 dòng.
        rt.anchorMin = rt.anchorMax = new Vector2(0.5f, 0f);
        rt.pivot     = new Vector2(0.5f, 1f);
        rt.anchoredPosition = new Vector2(0f, -2f);
        rt.sizeDelta = new Vector2(w + 24f, 26f);

        var txt = go.AddComponent<TextMeshProUGUI>();
        txt.fontSize          = 18;
        txt.enableAutoSizing  = true;          // [V6] tên dài tự co 18 → 12 rồi mới "…"
        txt.fontSizeMin       = 12;
        txt.fontSizeMax       = 18;
        txt.maxVisibleLines   = 1;             // [V6] đúng 1 dòng
